...
namespace
{
    ...
    size_t DER_read_length(size_t &offset, std::vector<uint8_t> &b64_decoded)
    {
        if (b64_decoded[offset] != 2)
            return std::cerr << "expected integer tag, but received: " << (int)b64_decoded[offset] << "\n", false;
        offset += 1;
        ...
        if (length_tag <= 0x7f)
            return length_tag;
        if (length_tag == 0x81)
            return length = b64_decoded[offset], offset += 1, length;
        if (length_tag == 0x82)
            return length = (b64_decoded[offset] << 8U) | (b64_decoded[offset + 1]), offset += 2, length;
        if (length_tag == 0x83)
            return length = (b64_decoded[offset] << 16U) | (b64_decoded[offset + 1] << 8U) | (b64_decoded[offset + 2]), offset += 3, length;
        if (length_tag == 0x84)
            return length = (b64_decoded[offset] << 24U) | (b64_decoded[offset + 1] << 16U) | (b64_decoded[offset + 2] << 8U) | (b64_decoded[offset + 3]),
                   offset += 4, length;
        return std::cerr << "error: bad length tag " << (int)length_tag << "\n", 1;
    }
    ...
    void DER_put_length(std::vector<uint8_t> &bytes, const size_t byte_length)
    {
        if (byte_length <= 0x7f)
            ...
            bytes.push_back(0x81), bytes.push_back(byte_length);
        else if (byte_length <= 0xffff)
            bytes.push_back(0x82), bytes.push_back(byte_length >> 8U), bytes.push_back(byte_length & 255);
        else if (byte_length <= 0xffffff)
            bytes.push_back(0x83), bytes.push_back(byte_length >> 16U), bytes.push_back((byte_length >> 8U) & 255), bytes.push_back(byte_length & 255);
        else if (byte_length <= 0xffffffff)
            bytes.push_back(0x84), bytes.push_back(byte_length >> 24U), bytes.push_back((byte_length >> 16U) & 255), bytes.push_back((byte_length >> 8U) & 255),
    ...
    void serialize_tail(std::vector<uint8_t> &bytes, const MPA::Integer<word_t> &x)
    {
        for (size_t i = x.get_head() - 1; i < x.get_head(); --i)
        {
            ...
            while (shift >= 0)
                bytes.push_back((word >> shift) & 0xFF), shift -= 8;
        }
    }
        ...
        for (size_t j = 0; j < bits_in_word / 8; ++j)
        {
            if (((leading_word >> ((bits_in_word - 8) - j * 8)) & 255) || j == bits_in_word / 8 - 1)
            {
                ...
                if (MSB_set) // in this case we prepend a zero byte
                    DER_put_length(bytes, bytesize + 1), bytes.push_back(0);
                else
...
bool parse_rsa_private_key(const std::vector<uint8_t> &rsa_private_key_bytes, RSA &rsa)
{
    ...
    if (!memcmp(openssh_identifier, b64_decoded.data(), 15))
    {
        ...
        size_t key_count = (b64_decoded[offset] << 24U) |
                           ...
                           (b64_decoded[offset + 3]);
        if (key_count != 1)
            return std::cerr << "ERROR: expected key count to be 1, but read " << key_count << "\n", false;
        offset += 4;
        ...
        if (pub_identifier_length != 7 || memcmp(identifier, &b64_decoded[offset], 7))
            return std::cerr << "ERROR: embedded public key format not supported\n", false;
        offset += 7;
        ...
        if (decryption_exponent != MPA::modular_inverse(encryption_exponent, (prime1 - 1) * (prime2 - 1)) ||
            ...
            return std::cerr << "ERROR: bad private key, required component relations don't hold !\n", false;
        return rsa = RSA(modulus, prime1, prime2, encryption_exponent, decryption_exponent), true;
    }
    ...
    if (version_length > 1 || b64_decoded[offset] != 0)
        return std::cerr << "unsupported version: " << (int)b64_decoded[offset] << "\n", false;
    offset += 1;
    // parse modulus -> n
    ...
    if (modulus != prime1 * prime2)
        return std::cerr << "bad private key: modulus does not match p and q!\n", false;
    if (!MPA::is_probably_prime(prime1) || !MPA::is_probably_prime(prime2))
        return std::cerr << "bad private key: p or q are not prime !\n", false;
    if (exponent1 != decryption_exponent % (prime1 - 1) || exponent2 != decryption_exponent % (prime2 - 1))
        return std::cerr << "bad private key: decryption exponent relations don't hold!\n", false;
    if (MPA::modular_inverse(prime2, prime1) != coefficient)
        return std::cerr << "bad private key: coefficient relations don't hold!\n", false;
    return rsa = RSA(modulus, prime1, prime2, encryption_exponent, decryption_exponent), true;
}
...
bool read_rsa_public_key_file(const std::string &filepath)
{
    ...
    return std::cerr << "ERROR: cannot parse public key\n", false;
}
...
bool read_rsa_private_key_file(const std::string &filepath)
{
    ...
    std::string line;
    if (!file.is_open())
        return std::cerr << "ERROR! Unable to open rsa private key file!\n", false;
    std::vector<uint8_t> b64;
    while (getline(file, line))
    {
...
bool read_rsa_key_file(const std::string &filepath)
{
    ...
    std::string line;
    if (!file.is_open())
        return std::cerr << "ERROR! Unable to open rsa key file!\n", false;
    getline(file, line);
    file.close();
    if (line.find("PRIVATE KEY") != std::string::npos)
        return read_rsa_private_key_file(filepath);
    else
...
int main(int argc, char **argv)
{
    ...
    if (argv[1] == generate)
    {
        ...
        if (ec)
            return std::cerr << "ERROR trying to set permissions for RSA private key file 'example.rsa'.\n",
        ...
        return std::cout << "wrote " << bytes_written << " bytes in total\n\n", 0;
    }
    // or if we read a key
